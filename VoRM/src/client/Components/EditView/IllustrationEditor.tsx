...
export function IllustrationEditor({ id, initialData, initialIsPublic, initialName, initialDescription, onCreated }: IllustrationEditorProps) {
  ...
  const { statusOfSave, saveMetadata, saveVisibility, saveGraph, isFull } = useSaveHandler({id, token: token ?? '', name, description, public:visibility=="public", saveTarget, onCreated})
